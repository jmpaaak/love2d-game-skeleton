...
LANE_LABEL="com.jm.${PROJECT_NAME}.${LANE_NAME}-lane"
{
  ...
  printf '**중요 — 거대 단일 파일 금지 + INBOX 최대 병렬:** docs/MODULE_STRUCTURE.md. 800줄/80KB Lua에 기능 붙이지 마라. 슬라이스=모듈 1개. 처리 대기를 파일이 안 겹치는 단위로 최대로 워크트리 병렬화. JSON/tools/순수 game 모듈은 즉시 WT.\n\n'
