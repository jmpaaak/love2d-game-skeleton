...
- 레인은 **파일/모듈 단위로 겹치지 않게** 나눈다 — 같은 파일을 두 레인이
  ...
  레인 vs 월드/경제 로직 레인.
- **INBOX 최대 병렬 (2026-09-07):** 처리 대기 항목을 담당 모듈이 안 겹치는
  단위로 **가능한 한 많이** 워크트리로 올린다. 거대 씬 파일(`play.lua` 등)을
  만지는 항목은 그 파일을 모듈로 쪼개기 전까지 직렬. JSON/`tools/`/순수
  `game/*.lua`는 모듈화를 기다리지 말고 즉시 WT. 상세: `docs/MODULE_STRUCTURE.md`.
